...
# SKIP THIS GO STRAIGHT TO RA defs
    ...
    # 	AREAM2 = raster::area(shp_expl)) # raster area function removes holes in polygons
    # 
...
names(presPolys) <- tolower(names(presPolys))
presPolys$ra <- factor(tolower(as.character(presPolys$ra)))
presPolys$aream2 <- area(presPolys)
...
# merge to get minSamps
presPolys <- merge(presPolys, EObyRA, all.x = TRUE, by = "ra")
presPolys$finalSampNum <- ifelse(presPolys$PolySampNum < presPolys$minSamps,
                                 presPolys$minSamps,
...
# write polys
nm.PyFile <- paste(sppCode, "_cleanpolys", sep = "")
presPolys$finalsamps <- presPolys$finalSampNum
colsToKeep <- c("poly.id", tolower(desiredCols), "finalsamps")
writeOGR(presPolys[colsToKeep], dsn = ".", layer = nm.PyFile, driver="ESRI Shapefile", overwrite_layer=FALSE)

# merge poly IDs to points, add columns for backwards compatability
ranPts2 <- merge(ps, presPolys@data , by = "poly.id")
ranPts2$stratum <- paste("poly_",ranPts2$poly.id, sep="") # this used to be panelNum column...not used so dropped
ranPts2$siteID <- paste(sppCode, ranPts2$poly.id, sep = "-")

## testing if force.min can be false and add points manually if needed
# need <- aggregate(presPolys$minSamps, by = list(presPolys$eo_id_st), max)
# names(need) <- c("eo", "countneed")
# have <- table(ranPts2$eo_id_st)
# have <- data.frame(eo = names(have), counthave = as.vector(have))
# comb <- merge(need, have, "eo")
# left <- comb[comb$countneed > comb$counthave,]
  ...
  # # skip to here if using systematic sampling update
...
# skip to here if using systematic sampling update
names(ranPts2@data) <- tolower(names(ranPts2@data))
...
colsToKeep <- c("siteid", "stratum", tolower(desiredCols))
ranPts2 <- ranPts2[,colsToKeep]

writeOGR(ranPts2, dsn = fullName, layer = nm.RanPtFile, 
         driver="ESRI Shapefile", overwrite_layer=TRUE)
ranPts <- ranPts2
...
OutPut <- data.frame(SciName = paste(ranPts$sname[1]),
	CommName=paste(ranPts$scomname[1]),
	ElemCode=sppCode,
...
backgShapef$insideBuff <- coincidentPts
